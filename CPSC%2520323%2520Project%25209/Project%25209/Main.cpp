...
//Functions
void read(stack<string>& st, queue<char>& input, string table[17][11], char
	key[11]);
void p(stack<string>& st, queue<char>& input, string table[17][11], char
...
void read(stack<string>& st, queue<char>& input, string table[17][11], char key[11]) {
...
void displayStack(stack<string>& st)
{
	...
	stack<string> copySt = st;
	stack<string> finalSt;
	string temp;
	...
	while (!finalSt.empty())
	{
		temp = finalSt.top(); //sets temp to the char on top of the stack
		cout << temp << " "; //prints out temp
		finalSt.pop(); //removes the top char from the stack
	}
	cout << endl;
}
